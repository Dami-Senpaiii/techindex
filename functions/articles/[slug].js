...
<body>
  <header class="nav"><div class="container nav-inner"><a class="brand" href="/"><span class="brand-badge" aria-hidden="true"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="4" ry="4"/><path d="M7 16l5-8 5 8" fill="none"/></svg></span><span>TechIndex</span></a><nav class="menu" aria-label="Hauptnavigation"><a href="/">Magazin</a><a href="/about.html">Über uns</a></nav></div></header>
  <main><article class="post container"><header class="post-header"><span class="post-tag">${escapeHtml(article.category)}</span><h1>${escapeHtml(article.title)}</h1><p class="post-meta">Aktualisiert: ${formatDate(article.date)} - Lesezeit: ${escapeHtml(article.readingTime)}</p><p class="lead">${escapeHtml(article.excerpt || description)}</p></header><figure class="hero"><img src="${escapeHtml(imageSrc(article))}" alt="${escapeHtml(article.imageAlt || article.title)}" loading="lazy"><figcaption>${escapeHtml(article.imageAlt || article.title)}</figcaption></figure><section>${renderContent(article.content)}</section><footer class="post-footer"><a class="back-link" href="/">Zurück zum Magazin</a></footer></article></main>
